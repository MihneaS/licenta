...
Mesh* Laborator6::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
	unsigned int VAO = 0;
...
void Laborator6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
	...
	location1 = glGetUniformLocation(shader->program, "Model");
	...
	location2 = glGetUniformLocation(shader->program, "View");
	...
	location3 = glGetUniformLocation(shader->program, "Projection");
