...
void BaseScene::InitResources()
{
	...
	xozPlane = new Mesh(MeshId::plane);
...

	{
		...
		simpleLine = new Mesh(MeshId::line);
		rm->StoreMesh(MeshId::line, simpleLine);
...
//{
//	ResourceManager::GetInstance()->StoreMesh(mesh);
//}

void BaseScene::DrawCoordinatSystem()
{
	DrawCoordinatSystem(camera->GetViewMatrix(), camera->GetProjectionMatrix());
}

void BaseScene::DrawCoordinatSystem(const glm::mat4& viewMatrix, const glm::mat4& projectionMaxtix)
{
	ResourceManager* rm = ResourceManager::GetInstance();
	...
	{
		Shader* shader = rm->GetShader(ShaderId::color);
		shader->Use();
...
void BaseScene::RenderMesh(Mesh* mesh, Shader* shader, glm::vec3 position, glm::vec3 scale)
{
...
void BaseScene::RenderMesh(Mesh* mesh, glm::vec3 position, glm::vec3 scale)
{
	ResourceManager* rm = ResourceManager::GetInstance();
	RenderMesh(mesh, rm->GetShader(ShaderId::simple), position, scale);
}

void BaseScene::RenderMesh2D(Mesh* mesh, Shader* shader, const glm::mat3& modelMatrix)
{
	if (!mesh || !shader || !shader->program)
		return;
...
void BaseScene::RenderMesh2D(Mesh* mesh, const glm::mat3& modelMatrix, const glm::vec3& color) const
{
	ResourceManager* rm = ResourceManager::GetInstance();
	Shader* shader = rm->GetShader(ShaderId::color);
...
void BaseScene::RenderMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
	if (!mesh || !shader || !shader->program)
		return;
...
void BaseScene::ReloadShaders() const
{
	ResourceManager* rm = ResourceManager::GetInstance();
	rm->ReloadShaders();
}

Camera* BaseScene::GetSceneCamera() const
{
	return camera;
}

InputController* BaseScene::GetCameraInput() const
{
	return cameraInput;
}
